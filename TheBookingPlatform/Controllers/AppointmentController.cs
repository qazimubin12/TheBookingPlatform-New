    ...
    public class AppointmentController : Controller
    {
                ...
                {  
                    if (item.IsPaid == false && item.DepositMethod == "Online" && item.IsCancelled == false && (DateTime.Now - item.BookingDate).TotalMinutes > 15)
                    {
                        ...
                        if (googleCalendar != null && !googleCalendar.Disabled)
                        {
                            try
                            {
                                var url = new System.Uri("https://www.googleapis.com/calendar/v3/calendars/" + employee.GoogleCalendarID + "/events/" + item.GoogleCalendarEventID);
                    ...
                    else
                    {
                        ...
                        AppointmentModel.Add(new AppointmentModel { IsRepeat = item.IsRepeat, AnyEmployeeSelected = item.AnyAvailableEmployeeSelected, NewCustomer = NewCustomer, Date = item.Date, AppointmentEndTime = item.EndTime, Time = item.Time, Color = item.Color, ID = item.ID, Notes = item.Notes, IsPaid = item.IsPaid, IsCancelled = item.IsCancelled, EmployeeID = item.EmployeeID, CustomerFirstName = customer.FirstName, CustomerLastName = customer.LastName, MobileNumber = customer.MobileNumber, Services = serviceList, TotalDuration = TotalDuration, ReminderSent = remindershouldbeSent && item.Reminder });
            ...
            else
            {
                ...
                foreach (var item in appointments)
                {
                    ...
                    else
                    {
                        ...
                        AppointmentModel.Add(new AppointmentModel { IsRepeat = item.IsRepeat, Date = item.Date, AppointmentEndTime = item.EndTime, Time = item.Time, Color = item.Color, ID = item.ID, Notes = item.Notes, IsPaid = item.IsPaid, IsCancelled = item.IsCancelled, EmployeeID = item.EmployeeID, CustomerFirstName = customer.FirstName, CustomerLastName = customer.LastName, MobileNumber = customer.MobileNumber, Services = serviceList, TotalDuration = TotalDuration, ReminderSent = remindershouldbeSent && item.Reminder });
        ...
        public ActionResult AppointmentDetailsPartial(int AppointmentID)
        {
            ...
            if (customer != null)
            {
                ...
                model.Histories = HistoryServices.Instance.GetHistoriesWRTBusinessandCustomer(LoggedInUser.Company, customer.FirstName + " " + customer.LastName);
        ...
        public ActionResult UpdateAppointment(string id, string start, string end, string EmployeeID)
        {
            ...
            if (googleCalendar != null && !googleCalendar.Disabled)
            {
                ...
                //var url = new System.Uri("https://www.googleapis.com/calendar/v3/calendars/" + oldEmployeeCalendarId + "/events/" + appointment.GoogleCalendarEventID);
                //var restClient = new RestClient(url);
                //var request = new RestRequest();
                ...
                {
                    GenerateonGoogleCalendar(appointment.ID, ConcatenatedServices);
        ...
        [HttpGet]
        public JsonResult CheckRunTimeSlot(string business, string timeslot, string Date, string serviceDuration, int EmployeeID)
        {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    RefreshToken(LoggedInUser.Company);
                                    mesage = GenerateonGoogleCalendar(appointment.ID, ConcatenatedServices);
                                }
                    ...
                    if (model.AllEmployees == true)
                    {
                        foreach (var emp in model.Employees)
                        {
                            ...
                            if (googleCalendar != null && !googleCalendar.Disabled)
                            {
                                ...
                                {
                                    GenerateonGoogleCalendar(appointment.ID, service);
                                }
                            }
        ...
        {
            var appointmentID = state.Split('_')[1];
            var appointment = AppointmentServices.Instance.GetAppointment(int.Parse(appointmentID));
            int year = appointment.Date.Year;
            int month = appointment.Date.Month;
            int day = appointment.Date.Day;
            int starthour = appointment.Time.Hour;
            ...
            try
            {
                var services = state.Split('_')[0];

                if (string.IsNullOrWhiteSpace(error))
                {
                    string tokensResult = this.GetTokens(code);
                    if (tokensResult == "Tokens Updated")
                    {
                        ...
                        if (response.StatusCode == System.Net.HttpStatusCode.OK)
                        {
                            ...
                            appointment.GoogleCalendarEventID = jsonObj["id"].ToString();
                            AppointmentServices.Instance.UpdateAppointment(appointment);
                            return RedirectToAction("Calendar", "Appointment", new { Date = startDateNew.ToString("yyyy-MM-dd") });
                        }
                        else
                        {
                            // Log error details
                            System.Diagnostics.Debug.WriteLine("Error: " + response.Content);
                            Session["ErrorOfAPI"] = response.Content;
                    ...
                    {
                        // Log error details
                        ViewBag.ErrorMessage = tokensResult;
                        Session["ErrorOfAPI"] = tokensResult;
        ...
        public string GetTokens(string code)
        {
            ...
            request.AddParameter("redirect_uri", "https://app.yourbookingplatform.com/Appointment/CallBack");
            ...
            else
            {
                // Log error details
                System.Diagnostics.Debug.WriteLine("Error: " + response.Content);
                return "Tokens Failed to Update: " + response.Content;
            }
        }
        ...
        private int CreateAppointment(int employeeId, DateTime date, string time, int serviceId, string notes, string color, User loggedInUser, Entities.Service service, bool IsRepeat, string frequency = "", int every = 0, string ends = "", int? numberofTimes = 0, string SpecificEndDate = "", int RepeatedAppoitnmentID = 0)
        {
            var appointment = new Appointment
            {
            ...
            if (googleCalendar != null && !googleCalendar.Disabled)
            {
                string message = RefreshToken(loggedInUser.Company);
        ...
        private void ScheduleEvent(string frequency, int every, List<string> daysOfWeek, string ends, int? numberOfTimes, DateTime? specificEndDate,
            ...
            if (frequency == "Every Week")
            {
                while (true)
                {
                    ...
                    else
                    {
                        ...
                        if (Occurences == 1)
                        {
                            var ID = CreateAppointment(employeeId, nextEventDate, time, serviceId, notes, color, loggedInUser, service, true, frequency, every, ends, numberOfTimes, specificEndDate.ToString(), FirstAppointmentID);
